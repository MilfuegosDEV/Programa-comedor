...
class IngresoMenú:
    ...
    def enter (self, event):
        ...
        try:
            self.DNI = ((self.entry.get()).upper()).strip()
        except:
            self.DNI = (self.entry.get()).strip()
            ...
            if self.DNI.isalpha() == True or self.DNI.isalnum() == False:
                ...
                if self.DNI not in self.__hoy:
                    ...
                    if self.DNI in self.info.keys():
                        ...
                        self.entry.delete(0, END)
                        VentanaColores(self.RegistraCedulas, resource_path(r'src\resources\images\PantallaNoEstaBecado.png'))

                else:
                    self.entry.delete(0, END)
                    VentanaColores(self.RegistraCedulas, resource_path(r'src\resources\images\PantallaYaUtilizoElComedor.png'))
